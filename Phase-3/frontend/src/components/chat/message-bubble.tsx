...
export function MessageBubble({ role, content, timestamp }: MessageBubbleProps) {
  ...
  return (
    <div className={`flex gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
      ...
      <div className={`max-w-[85%] rounded-2xl ${isUser ? "ml-auto rounded-br-md bg-primary-500 text-white" : "rounded-tl-md bg-secondary-100 dark:bg-secondary-800"}`}>
        <div className="px-4 py-3 whitespace-pre-wrap">{content}</div>
        {timestamp && <div className={`px-4 pb-2 text-xs ${isUser ? "text-primary-200" : "text-secondary-500"}`}>{timestamp}</div>}
      </div>
      {isUser && (
